...
📐 [Architecture](docs/ARCHITECTURE.md) · 🧭 [ADRs](docs/adr/) · 📝 [Case study](docs/CASE_STUDY.md) · 🔌 [Postman collection](docs/ComplianceAgent.postman_collection.json) · 🗂️ [Model card](docs/MODEL_CARD.md) · 🧪 [Model validation](docs/MODEL_VALIDATION.md) · 🏛️ [Model governance](docs/MODEL_GOVERNANCE.md) · [Data sheet](docs/DATA_SHEET.md)
...
| **Regulatory-Context** | Ground it in policy | **Advanced RAG** over a ~112-chunk regulatory KB: **hybrid retrieval** (BM25 + dense embeddings, **RRF-fused**) → **reranking** (funnel), reporting **Recall@5 / MRR / nDCG@10**. Embeddings are pluggable — a **stronger $0 n-gram embedder** by default (word + char n-grams, TF-weighted; **+22% Recall@3 / +23% MRR** on paraphrased queries vs plain hashing), or **Gemini neural** (`EMBEDDING_BACKEND=gemini`); reranker is lexical (default) or **LLM** (`RERANKER=llm`). |
